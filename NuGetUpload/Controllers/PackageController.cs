...
public class PackageController : ControllerBase
{
    ...
    public PackageController(ILogger<PackageController> logger,
                             ...
                             GamePackageInfoService gamePackageInfoService)
    {
        _logger = logger;
    ...
    public async Task<ActionResult<GamePackageInfo>> UploadAsync(
        ...
        [FromForm(Name = "inputFile")] [Required]
        IFormFile[] files)
    {
        if (!NuGetVersion.TryParse(version, out var parsedVersion)) return BadRequest("Failed to parse version");

        if (files.Length > _packageUploadService.Options.MaxFiles) return BadRequest("Too many files");
        ...
        if (info == null) return Unauthorized("Invalid key");
        ...
        try
        {
            var packageInfo = await _packageUploadService.UploadPackage(info,
                files.Select(file => new InputFormFile(file)).Cast<IInputFile>().ToList(), parsedVersion, unityVersion,
                new Progress<(double Progress, string Status)>(x =>
                {
