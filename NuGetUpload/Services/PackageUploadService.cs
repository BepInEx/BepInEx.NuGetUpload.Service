...
public class PackageUploadService
{
    ...
    public async Task<PackageInfo> UploadPackage(GamePackageInfo info,
                                                 IList<IInputFile> files,
                                                 ...
                                                 IProgress<(double, string)> progress = null)
    {
        ...
        if (!info.SkipDuplicateMitigation)
        {
            ...
            if (highest != null)
            {
                ...
                if (index != -1) number = int.Parse(releaseLabels[index + 1]) + 1;
            }

            version = new NuGetVersion(version.Version, version.ReleaseLabels.Append("r").Append(number.ToString()),
                version.Metadata, version.OriginalVersion);
        }
